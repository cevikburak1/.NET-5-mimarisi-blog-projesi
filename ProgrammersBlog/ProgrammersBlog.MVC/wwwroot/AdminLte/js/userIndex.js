            ...
            placeHolderDiv.on('click',
                ...
                function (event) {
                    ...
                    $.ajax({
                        ...
                        contentType: false,
                        success: function (data) {
                            const userRoleAssignUpdateAjaxModel = jQuery.parseJSON(data);
                            console.log(userRoleAssignUpdateAjaxModel);
                            const newFormBody = $('.modal-body', userRoleAssignUpdateAjaxModel.RoleAssignPartial);
                            placeHolderDiv.find('.modal-body').replaceWith(newFormBody);
                            const isValid = newFormBody.find('[name="IsValid"]').val() === 'True';
                            if (isValid) {
                                const id = userRoleAssignUpdateAjaxModel.UserDto.User.Id;
                                const tableRow = $(`[name="${id}"]`);
                                //placeHolderDiv.find('.modal').modal('hide');

                                toastr.success(`${userRoleAssignUpdateAjaxModel.UserDto.Message}`,"Başarılı İşlem");
                            } else {
                                let summaryText = "";
                                $('#validation-summary > ul > li').each(function () {
                                    let text = $(this).text();
