    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            Swal.fire({
                ...
                text: `${categoryName} Kategori Arşivden Geri Getirilicektir!`,
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (data) {
                            const undoDeletedCategryresult = jQuery.parseJSON(data);
                            console.log(undoDeletedCategryresult);
                            if (undoDeletedCategryresult.ResultStatus===0) {
                                Swal.fire(
                                    'Arşivden Geri geldi!',
                                    `${undoDeletedCategryresult.Message}`,
                                    'success'
                                );
                                ...
                                Swal.fire({
                                    ...
                                    text: `${undoDeletedCategryresult.Message}`,
                                });
                            }
                        },
                        error: function (err) {
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            Swal.fire({
                ...
                text: `${categoryName} Kategori Tamemen Silicektir!`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Evet,Arşivden Silmek istiyorum.',
                cancelButtonText: 'Hayır,istemiyorum.'
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function (data) {
                            const harddelete = jQuery.parseJSON(data);
                            console.log(harddelete);
                            if (harddelete.ResultStatus === 0) {
                                Swal.fire(
                                    'Arşivden Geri geldi!',
                                    `${harddelete.Message}`,
                                    'success'
