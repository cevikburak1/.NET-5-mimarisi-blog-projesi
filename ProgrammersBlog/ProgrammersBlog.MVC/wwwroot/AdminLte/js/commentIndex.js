$(document).ready(function () {
                            ...
                            if (commentResult.Data) {
                                ...
                                $.each(commentResult.Data.Comments.$values,
                                    function (index, comment) {
                                        ...
                                        jqueryTableRow.attr('name', `${newComment.ıd}`);
                                    });
                                dataTable.draw();
                                $('.spinner-border').hide();
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (commentResult.Data) {
                                Swal.fire(
                                    'Silindi!',
                                    `${commentResult.Data.Comment.ıd} no luyorum başarı ile silindi`,
                                    'success'
                                );
            ...
            function (event) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        //    const tableRow = $(`[name="${id}"]`);
                        //}
                        const id = commentUpdateAjaxModel.CommentDto.Comment.ıd;
                        const tableRow = $(`[name="${id}"]`);
                        ...
                        if (isValid) {
                            ...
                            dataTable.row(tableRow).data([
                                commentUpdateAjaxModel.CommentDto.Comment.ıd,
                            ...
                            toastr.success(`${commentUpdateAjaxModel.CommentDto.Comment.ıd} no'lu yorum başarıyla güncellenmiştir`, "Başarılı İşlem!");
                        } else {
                            let summaryText = "";
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (commentResult.Data) {
                                ...
                                Swal.fire(
                                    'Onaylandı!',
                                    `${commentResult.Data.Comment.ıd} no'lu yorum başarıyla onaylanmıştır.`,
                                    'success'
                                );
    ...
    function getButtonsForDataTable(comment) {
        if (!comment.IsActive) {
            return `
                                <button class="btn btn-warning btn-sm btn-approve" data-id="${comment.ıd
                }"><span class="fas fa-thumbs-up"></span></button>
                                <button class="btn btn-info btn-sm btn-detail" data-id="${comment.ıd
                }"><span class="fas fa-newspaper"></span></button>
                                <button class="btn btn-primary btn-sm mt-1 btn-update" data-id="${comment.ıd
                }"><span class="fas fa-edit"></span></button>
                                <button class="btn btn-danger btn-sm mt-1 btn-delete" data-id="${comment.ıd
                }"><span class="fas fa-minus-circle"></span></button>
                                            `;
        }
        return `<button class="btn btn-info btn-sm btn-detail" data-id="${comment.ıd}"><span class="fas fa-newspaper"></span></button>
                                <button class="btn btn-primary btn-sm mt-1 btn-update" data-id="${comment.ıd}"><span class="fas fa-edit"></span></button>
                                <button class="btn btn-danger btn-sm mt-1 btn-delete" data-id="${comment.ıd}"><span class="fas fa-minus-circle"></span></button>`
